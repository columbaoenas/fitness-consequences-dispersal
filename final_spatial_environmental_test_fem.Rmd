---
title: "Spatial/Environmental Tests"
...
breeding_fem_habitat <- breeding_fem_habitat %>% 
...
```{r, echo = FALSE}
test_pop_imm <-  lmer(popularity_index ~ disp_fem + (1|mother) + (1|study_year), 
                           data = breeding_fem_habitat)
summary(test_pop_imm)
plot(predict_response(test_pop_imm, terms = "disp_fem"))
hist(residuals(test_pop_imm), breaks = 300)
...
test_altitude_imm <- lmer(altitude_m ~disp_fem + (1|mother) + (1|study_year), 
                           data = breeding_fem_habitat)
summary(test_altitude_imm)

plot(predict_response(test_altitude_imm, terms = c("disp_fem")))

hist(residuals(test_altitude_imm), breaks =300)
```



```{r, echo = FALSE}
#graph of altitude counts separated by disp_fem
altitude_hist <- ggplot(breeding_fem_habitat) + 
  geom_histogram(aes(x = altitude_m, fill = disp_fem)) + 
  theme(legend.position = "none") 

#proportion at each altitude of disp_fem
altitude_hist_fill <- ggplot(breeding_fem_habitat) + 
  geom_histogram(aes(x = altitude_m, fill = disp_fem), position = "fill") + 
...
Model: EDI ~ disp_fem + (1|study_year) + (1|mother)


```{r edge distance, echo = FALSE}
test_edge_imm <- lmer(edge_EDI ~ disp_fem + (1|study_year) + (1|mother), 
                           data = breeding_fem_habitat)
summary(test_edge_imm)

plot(predict_response(test_edge_imm, terms = "disp_fem"))

hist(residuals(test_edge_imm), breaks = 300)


test_edge_imm_sqrt <- lmer(sqrt(edge_EDI) ~ disp_fem + (1|study_year) + (1|mother), 
                           data = breeding_fem_habitat)
summary(test_edge_imm_sqrt)



hist(residuals(test_edge_imm_sqrt), breaks = 300)
...
edge_hist <- ggplot(breeding_fem_habitat) +
  geom_histogram(aes(x = edge_EDI, fill = disp_fem)) 


edge_hist_fill <- ggplot(breeding_fem_habitat) +
  geom_histogram(aes(x = edge_EDI, fill = disp_fem), position = "fill") + 
  geom_hline(yintercept = 0.5)
...
oaks_hist <- ggplot(breeding_fem_habitat) +
  geom_histogram(aes(x = oaks, fill = disp_fem), bins = 150) 


oaks_hist_fill <- ggplot(breeding_fem_habitat) +
  geom_histogram(aes(x = oaks, fill = disp_fem), position = "fill", bins = 150) + 
  geom_hline(yintercept = 0.5)
...
test_oaks_imm_norm <-  lmer(oaks ~ disp_fem + (1|mother) + (1|study_year), 
                           data = breeding_fem_habitat)
summary(test_oaks_imm_norm)

hist(residuals(test_oaks_imm_norm))

plot(predict_response(test_oaks_imm_norm, terms = "disp_fem"))
...
test_oaks_imm_pois <-  glmer(oaks ~ disp_fem + (1|mother) + (1|study_year), 
                           data = breeding_fem_habitat,
                           family = "poisson")
...
plot(predict_response(test_oaks_imm_pois, terms = "disp_fem"))
